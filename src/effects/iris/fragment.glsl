...
vec3 getSchemeColor(float g, float l, float t) {
    ...
    if (iColorScheme == 7) return getVaporwave(g, l, t);
    if (iColorScheme == 8) return getNeonFlux(g, l, t);
    if (iColorScheme == 9) return getMidnightFlux(g, l, t);
...
void mainImage(out vec4 fragColor, vec2 fragCoord) {
    ...
    float foldStrength = 0.15 + iAudioMomentum * 0.1 + iAudioBeatPulse * 0.2;
    ...
    float beatPush = (0.1 + iAudioBeatPulse * 0.4 + iAudioBass * 0.2) * flowDrive;
    ...
    float w = (0.06 + iAudioBeatPulse * 0.07 + bassEnergy * 0.05) * bandSharp;
    ...
    float dotSize = px * (1.5 + bassEnergy * 2.5) * dotBeat * dotGuard;
    ...
    float particleNoise = sin(particleAngle * 8.0 + t * 1.2 + particleRadius * 6.0);
    ...
    float particleEnergy = (0.25 + iAudioBeatPulse * 0.35 + iAudioTreble * 0.25) * densityControl;
    ...
    float irisFrequency = 8.0 + irisStrength * 6.0 + bassEnergy * 3.0;
    float flowOffset = flowDrive * 0.6 + iBeatZoom * 0.2;
    float irisTemporal = 0.5 + flowDrive * 0.4 + midEnergy * 0.5 + iAudioBeatPulse * 0.25;
    float irisWave = sin((l + flowOffset * t + beatPush) * irisFrequency - t * irisTemporal - iBeatRotation * 0.35);
    float irisMask = smoothstep(0.35, 0.95, abs(irisWave));
    float irisFeather = exp(-abs(irisWave) * (2.0 + irisStrength));
    float irisEnergy = (midEnergy + iAudioMid * 0.3 + irisStrength * 0.1 + flowDrive * 0.2);
    c += rgb * irisMask * 0.18 * irisEnergy;
    c += rgb * irisFeather * 0.12 * (bassEnergy + 0.3);
    ...
    float aberration = (0.002 + flowDrive * 0.001) + iAudioTreble * 0.0015;
    ...
    c = mix(c, aberrated, 0.25 + iAudioTreble * 0.25);
    ...
    glow *= 0.35 + iAudioBeatPulse * 0.45 + energyMix * 0.4;
    c += rgb * glow * 0.8;

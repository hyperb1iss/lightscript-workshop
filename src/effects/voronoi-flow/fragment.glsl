...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
  ...
  vec2 w1 = vec2(vnoise(uv*2.0 + iTime*0.3*speed), vnoise(uv*2.0 + 17.3 + iTime*0.29*speed));
  vec2 w2 = vec2(vnoise(uv*4.0 - iTime*0.41*speed), vnoise(uv*4.0 + 23.7 - iTime*0.37*speed));
  vec2 pw = uv*scale + swirl*0.6*(w1-0.5) + swirl*0.3*(w2-0.5);

  // Slight swirl by rotating proportional to noise
  float a = (vnoise(uv*3.0 + iTime*0.2*speed)-0.5) * 3.14159 * 0.25 * swirl;
  ...
  float spark = step(0.98, vnoise(pw*6.0 + iTime*1.7*speed)) * 0.5 * clamp(shimmer,0.0,2.0);

...
export class Particle {
    ...
    constructor(canvasWidth: number, canvasHeight: number, size: number, speedFactor: number, flowDirection: number) {
        ...
        this.setSpeedByDirection(canvasWidth, canvasHeight, speedFactor, flowDirection)
     ...
     */
    setSpeedByDirection(canvasWidth: number, canvasHeight: number, speedFactor: number, flowDirection: number): void {
     ...
     */
    update(canvasWidth: number, canvasHeight: number, speed: number, bounce: boolean): void {
        ...
        if (this.angleSpeed !== 0) {
            ...
            const baseSpeed = Math.sqrt(this.speedX * this.speedX + this.speedY * this.speedY)
            this.speedX = Math.cos(this.angle) * baseSpeed
            this.speedY = Math.sin(this.angle) * baseSpeed
        }
     ...
     */
    setColor(colorMode: number, time: number, saturation: number, intensity: number): void {
        let hue: number
        let sat: number
        let light: number
        ...
        switch (colorMode) {
            case 0: // Aurora
                ...
                hue =
                    ...
                    360
                sat = saturation * 0.9
                light = 50 + Math.sin(normalizedTime + (this.x + this.y) / 200) * 15 + (intensity - 100) / 5
            ...
            case 1: {
                ...
                hue = cyberColors[Math.floor((this.x + this.y + normalizedTime * 100) / 100) % cyberColors.length]
                sat = saturation
            ...
            case 2: // Fire and Ice
                if (this.y < this.x) {
                    ...
                    sat = saturation
                    light = 50 + Math.sin(normalizedTime * 3) * 10 + (intensity - 100) / 5
                } else {
                    hue = 200 + Math.sin(normalizedTime - this.x / 100) * 20 // Ice: blue
                    sat = saturation * 0.8
                    light = 60 + Math.sin(normalizedTime * 2) * 10 + (intensity - 100) / 5
                }
                break
            ...
            case 3: {
                ...
                const distFromCenter = Math.sqrt(
                    (this.x - window.innerWidth / 2) ** 2 + (this.y - window.innerHeight / 2) ** 2,
                )

                // Spiral pattern based on distance and angle
                const spiral = Math.atan2(this.y - window.innerHeight / 2, this.x - window.innerWidth / 2) * 10
                hue = (270 + spiral + distFromCenter / 5 + normalizedTime * 5 + offset * 0.3) % 360

                // Vary saturation based on distance from center
                sat = Math.min(saturation, 70 + distFromCenter / 8)
                light = 25 + Math.sin(normalizedTime + distFromCenter / 30) * 15 + (intensity - 100) / 5
            ...
            case 5: {
                ...
                const index = Math.floor(normalizedTime + this.x / 100 + this.y / 100) % neonColors.length
            ...
            case 6: // Ocean
                ...
                sat = saturation
                light = 40 + Math.sin(normalizedTime * 2 + this.y / 50) * 20 + (intensity - 100) / 5
                break
            ...
            case 8: {
                ...
                hue = (offset + (wave1 + wave2 + wave3) * 60 + normalizedTime * 20) % 360
            ...
            case 10: {
                ...
                if (verticalPos < 0.4) {
                    ...
                    hue = 30 - (verticalPos - 0.4) * 60 + Math.sin(normalizedTime * 0.5) * 10
            ...
            case 11: {
                ...
                light = 55 + Math.sin(normalizedTime * 0.8) * 10 + (intensity - 100) / 5
                blendModeOverride = 'screen'
                break
            }

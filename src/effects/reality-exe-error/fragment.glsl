...
vec3 dialog(vec2 uv, float t, float severity) {
  ...
  float title = rect(
    uv,
    pos + vec2(0.0, size.y * 0.28),
    vec2(size.x * 0.98, size.y * 0.12),
    0.03
  );
  ...
  col += vec3(0.08, 0.1, 0.13) * (border * 0.9);
  col += vec3(0.95) * (body * 0.6);
  col += vec3(0.1, 0.35, 0.85) * (title * 0.9);
...
void mainImage(out vec4 fragColor, vec2 fragCoord) {
  ...
  vec2 cam =
    0.04 *
    iParallax *
    vec2(sin(t * 0.31 * (1.0 + iMotion)), cos(t * 0.27 * (1.0 + iMotion)));
  vec2 uvs = uv + cam;

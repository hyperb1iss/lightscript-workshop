...
// Custom plugin for decorator support in dev mode
function lightscriptDecoratorsPlugin(): Plugin {
  return {
        ...
        if (path.endsWith('.ts') && fs.existsSync(path)) {
          try {
            ...
            if (content.includes('@Effect') || 
              ...
              console.log(`[LightScript] Detected change in decorator file: ${path}`);
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(fullSourcePath)) {
        ...
        let sourceContent = fs.readFileSync(fullSourcePath, 'utf-8');
          ...
          if (importPath.startsWith('.')) {
            ...
            if (fs.existsSync(importPathResolved)) {
              ...
              if (
                ...
                console.log(`Found potential implementation file: ${importPathResolved}`);
                filesToCheck.push({ path: importPathResolved, content: importContent });
              }
            }
          }
        }
        ...
        for (const file of filesToCheck) {
          ...
          if (effectDecorators.length > 0) {
            console.log(`Found ${effectDecorators.length} Effect decorators in ${file.path}`);
          ...
          if (controlCount > 0) {
            console.log(`Generated ${controlCount} control tags from ${file.path}`);
      ...
      console.error(`Error extracting decorator metadata for ${effect.id}:`, err);
    }
...
// Add a helper function at the bottom of the file before export default
// Add before the "export default defineConfig..." line
...
function extractAllDecorators(fileContent: string, decoratorName: string): DecoratorInfo[] {
  ...
  const decoratorRegex = new RegExp(`@${decoratorName}\\s*\\(\\s*(?:{([\\s\\S]*?)}|["']([\\s\\S]*?)["'])\\s*\\)\\s*([a-zA-Z0-9_]+)`, 'g');
  ...
  while ((match = decoratorRegex.exec(fileContent)) !== null) {
    try {
      ...
      if (match[1]) { // Object literal style: @Decorator({ ... })
        const optionsText = match[1];
        
        // Extract key-value pairs using more robust regex
        const keyValuePairs = optionsText.match(/([a-zA-Z0-9_]+)\s*:\s*(?:(?:\[([\s\S]*?)\])|(?:"([^"]*)")|(?:'([^']*)')|(?:([^,\s}]+)))/g) || [];
      ...
      } else if (match[2]) { // String literal style: @Decorator("...")
    ...
    build: effect ? {
      ...
      rollupOptions: {
        ...
        output: {
          ...
          globals: {
            // Remove or comment out globals
          }
        },
        // Remove the external array or set it to an empty array
